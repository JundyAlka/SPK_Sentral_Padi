            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse(array_slice($hasil, 0, 5) as $h)
                        <tr>
                            <td class="px-4 py-6 whitespace-nowrap">
                                <span class="inline-flex items-center justify-center h-6 w-6 rounded-full {{ $h['rank'] == 1 ? 'bg-yellow-100 text-yellow-800' : ($h['rank'] == 2 ? 'bg-gray-100 text-gray-800' : ($h['rank'] == 3 ? 'bg-orange-100 text-orange-800' : 'bg-emerald-50 text-emerald-800')) }} text-xs font-bold">
                                    {{ $h['rank'] }}
                                </span>
                            </td>
                            <td class="px-4 py-6 whitespace-nowrap text-sm font-medium text-gray-900">
                                {{ $h['nama_daerah'] }}
                                <span class="block text-xs text-gray-500">{{ $h['provinsi'] }}</span>
                            </td>
                            <td class="px-4 py-6 whitespace-nowrap text-right text-sm font-bold text-emerald-600">
                                {{ number_format($h['skor_total'], 4) }}
                            </td>
                        </tr>
                        @empty
                        <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const hasil = @json(array_slice($hasil, 0, 10)); // Top 10 for chart
