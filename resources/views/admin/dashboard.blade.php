...
@section('title', 'Dashboard Admin - SPK Padi')
    ...
    <div class="relative bg-emerald-900 rounded-2xl shadow-xl mb-10 overflow-hidden min-h-[250px] flex items-center">
        ...
        <div class="relative z-10 px-8 py-10 w-full flex items-center justify-between">
            ...
            <div class="hidden md:block">
                 <img src="{{ asset('logo_stats.png') }}" class="w-40 h-40 object-contain drop-shadow-2xl opacity-90 hover:opacity-100 transition-opacity duration-500 animate-float" alt="Admin Icon">
            </div>
        </div>
    </div>
        ...
        <div>
            <div class="bg-white rounded-xl shadow overflow-hidden h-full">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse(collect($hasil)->take(5) as $row)
                            <tr class="{{ $loop->first ? 'bg-yellow-50' : '' }} hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="flex items-center justify-center w-8 h-8 rounded-full {{ $loop->first ? 'bg-yellow-400 text-white font-bold' : ($loop->iteration <= 3 ? 'bg-gray-200 text-gray-700' : 'text-gray-500') }}">
                                        {{ $row['rank'] }}
                                    </span>
                                </td>
                ...
                <div class="px-6 py-3 bg-gray-50 border-t border-gray-200 text-right">
                    <a href="{{ route('admin.daerah.index') }}" class="text-sm font-medium text-emerald-600 hover:text-emerald-800">Lihat Semua Data &rarr;</a>
                </div>
            </div>
        </div>
