    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse(collect($hasil)->take(5) as $index => $row)
                <tr class="hover:bg-emerald-50/50 transition duration-150 ease-in-out {{ $index == 0 ? 'bg-emerald-50' : '' }}">
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center h-8 w-8 rounded-full font-bold {{ $index == 0 ? 'bg-yellow-100 text-yellow-700 ring-4 ring-yellow-50' : ($index == 1 ? 'bg-gray-100 text-gray-700' : ($index == 2 ? 'bg-orange-100 text-orange-700' : 'text-gray-500')) }}">
                            {{ $row['rank'] }}
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-bold text-gray-900">{{ $row['nama_daerah'] }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $row['provinsi'] }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <span class="text-base font-bold text-emerald-700">{{ number_format($row['skor_total'], 4) }}</span>
                    </td>
                </tr>
                @empty
                <tr>
